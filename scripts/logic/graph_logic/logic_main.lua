...
splasher_location = {}
splasher_location.__index = splasher_location
...
---@type table<string, splasher_new_return>
NAMED_LOCATIONS = {}
local stale = true
...
---@field exits table<integer, {[1]:splasher_new_return, [2]:fun(): accessibilityLevel}>
---@field keys integer
...
---@param name string
---@return splasher_new_return
function splasher_location.new(name)
	if name == nil then
		error("splasher_location name cannot be nil")
	end
    local self = setmetatable({}, splasher_location)
	self.name = name
...
---@param exit string|splasher_new_return splasher_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function splasher_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = splasher_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
---@param exit string|splasher_new_return splasher_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function splasher_location:connect_two_ways(exit, rule)
...
---@param exit string|splasher_new_return splasher_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function splasher_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
---@param exit string|splasher_new_return splasher_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function splasher_location:connect_two_ways_entrance(name, exit, rule)
...
function splasher_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
---@param rule1? fun(): accessibilityLevel|boolean
---@param rule2? fun(): accessibilityLevel|boolean
function splasher_location:connect_two_ways_stuck(exit, rule1, rule2)
...
---@return accessibilityLevel
function splasher_location:accessibility()
...
---@param keys integer
function splasher_location:discover(accessibility, keys)
...
Entry_Point = splasher_location.new("Entry_Point")
